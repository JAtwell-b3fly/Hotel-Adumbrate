...
const AddRooms = () => {
    ...
    const validateForm = () => {
        ...
        if (!suiteType || suiteType === "") {
            console.error("Suite Type is required");
            isValid = false;
        }

        //Validate Room Number
        if (!roomNumber || roomNumber === "" || !/Room \d+/.test(roomNumber) || !/room \d+/.test(roomNumber))  {
            console.error("Room Number must contain the word 'Room' followed the number");
            isValid = false;
        } 

        //Validate Floor Level
        if (!floorLevel || floorLevel <= 0) {
            console.error("Floor Level is required");
            isValid = false;
        } 

        //Validate Price
        if (!price || price <= 0 || !/^\d+(\.\d{1,2})?$/.test(price)) {
            console.error("Price must be a positive number with up to two decimal places");
            isValid = false;
        } 

        //Validate Beds
        if (!beds || beds <= 0) {
            console.error("Beds must be a positive number");
            isValid = false;
        } 

        //Validate Bathrooms
        if (!bathrooms || bathrooms <= 0) {
            console.error("Bathrooms must be a positive number");
            isValid = false;
        } 
        ...
        //Validate Television
        if (!television || television <= 0) {
            console.error("Television must be a positive number");
            isValid = false;
        } 

        //Validate AirConditioning
        if (!airConditioning || airConditioning <=0) {
            console.error("Air Conditioning is required and must be a positive number");
            isValid = false;
        }
        ...
        //Validate Image
        if (!image.trim()) {
            console.error("Image URL cannot be an empty string");
            isValid = false;
        }
    ...
    const addNewRoom = async (event) => {
        ...
        try {
            let uniqueNumber;
                ...
                uniqueNumber = Math.floor(Math.random() * 1000) + 1;

                //Create the final document ID with the suite type and unique number
                const docId = `${suiteType.toLowerCase()}_room_${uniqueNumber}`;

                //Try to add the new room
                const roomRef = doc(collection(db, "availableRooms"),docId);
            ...
            console.log("Document already exists with docId: ", docId);
        }

        } catch (error) {
            console.error("Error add the new room: " + error.message);
        }
    };
    
    return(
        <div className="addrooms">
            <h1 className="add_room_heading" style={{color: "white"}}>Add Room</h1>
             <form className="form_div" onSubmit={(e) => {e.preventDefault(); addNewRoom();}}  style={{ maxHeight: "50rem", width: "66rem", overflowY: "auto" }}>
                <div style={{display:"flex", flexDirection:"row"}}>
                <div style={{display:"flex", flexDirection:"column"}}>
                <div className="input_div">
                    <label className="reservation_label" style={{color: "white", textAlign: "center"}}>Suite Type</label>
                    <select
                        name="suiteType"
                        //value={suiteType}
                        //defaultValue="Single"
